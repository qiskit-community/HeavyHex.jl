...
function setup_job(dec::OnlineMLDecoder)
    dec.prefix_store = ConcurrentCollections.ConcurrentDict{keytype(_hyperedges),Any}()
end
...
function decode(decoder::OnlineMLDecoder, observation::Tbs) where {Tbs}
    ...
    for j = _bit:(len_obs-1)
        ...
        for k in reverse(eachindex(_keys))
        ...
        _keys = newkeys
        jdistribution =
            Dictionary(Dict(key => decoder.numerical_hyperedges[key] for key in jkeys))
        ...
        if j < decoder.prefix_length
            # the value for any key is predetermined so it's fine if two separate threads both write to the same key
...
function update(pdict, newdict, hyperedge)
    ...
    for k in keys(pdict)
        ...
        if newk in keys(newdict)
            Dictionaries.set!(newdict, newk, newdict[newk] + pdict[k] * p)
        else
            val = pdict[k] * p
